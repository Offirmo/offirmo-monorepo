...
function pick_random_rank(rng: RNGEngine): MonsterRank {
	...
	switch(get_random.generator_of.integer.between(1, 10)(rng)) {
		case 1:
...
function create(rng: RNGEngine, hints: Readonly<Partial<Monster>> = {}): Monster {
	const raw = hints.name
		...
		: get_random.picker.of(ENTRIES)(rng)
	...
	let level = -1
	if (!hints.level)
		level = get_random.generator_of.integer.between(1, MAX_LEVEL)(rng)
	else {
		...
		level = Math.max(1, Math.min(MAX_LEVEL,
			reference_level + get_random.generator_of.integer.between(-variation, variation)(rng),
		))
	}
