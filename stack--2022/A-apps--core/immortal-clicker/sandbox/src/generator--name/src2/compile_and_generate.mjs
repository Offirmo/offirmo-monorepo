...
function latinize(str) {
	if (typeof str === 'string') {
		return str.replace(/[^A-Za-z0-9]/g, function(x) {
			return latinize_characters[x] || x;
		});
	} else {
		return str;
	}
...
;(() => {
	const raw_content = fs.readFileSync('./src/generator--name/src2/given-names--RBRB.txt', 'utf8')
	...
	// parsing: cleaning unneeded infos
	lines = lines
		.map(NORMALIZERS.coerce_to_ascii)
		.filter(l => !'0123456789'.includes(l[0]))
		.filter(l => {
			switch (l) {
				case 'M':
				case 'F':
