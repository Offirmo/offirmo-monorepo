	...
	describe('engines', function() {
		...
		describe('ISAAC-32', function() {
			...
			function toHex(n: number, padding: number = 8): string {
				...
				while(result.length < padding) result = '0' + result;
				return result
			}
...
/*
			...
			it('should pass the official unit tests -- randvect', () => {
				...
				// discarded
				const prng_1_results = _internal_result_toHex(s.results)

				// trigger another prng
				for(let i = 0; i < 256 ; ++i) {
					engine.get_Int32()
				}
				const prng_2_results = _internal_result_toHex(s.results)
				//console.log(prng_2_results)

				for(let i = 0; i < 256 ; ++i) {
					engine.get_Int32()
				}
				const prng_3_results = _internal_result_toHex(s.results)
				//console.log(prng_3_results)

				expect(prng_2_results + prng_3_results).to.equal(
