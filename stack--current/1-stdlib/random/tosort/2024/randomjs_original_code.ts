...
Random.engines = {
	...
	mt19937: (function (Int32Array) {
		...
		function seedWithArray(data, source) {
			...
			for (;
				(k | 0) > 0; --k) {
				data[i] = previous = ((data[i] ^ Math.imul((previous ^ (previous >>> 30)), 0x0019660d)) + (source[j] | 0) + (j | 0)) | 0;
				i = (i + 1) | 0;
				++j;
				if ((i | 0) > 623) {
					data[0] = data[623];
			...
			for (k = 623;
			     (k | 0) > 0; --k) {
				data[i] = previous = ((data[i] ^ Math.imul((previous ^ (previous >>> 30)), 0x5d588b65)) - i) | 0;
				i = (i + 1) | 0;
				if ((i | 0) > 623) {
					data[0] = data[623];
					i = 1;
		...
		function mt19937() {
			...
			next.seed = function (initial) {
				...
				for (var i = 1; i < 624; i = (i + 1) | 0) {
					data[i] = previous = (Math.imul((previous ^ (previous >>> 30)), 0x6c078965) + i) | 0;
				}
				index = 624;
				uses = 0;
				return next;
